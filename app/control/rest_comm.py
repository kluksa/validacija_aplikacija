...
class DataReaderAndCombiner(object):
    ...
    def get_data(self, kanal, od, do):
        ...
        try:
            ...
            master_konc_frm = pd.DataFrame(columns=self.citac.expectedColsKonc)
            master_zero_frm = pd.DataFrame(columns=self.citac.expectedColsZero)
            master_span_frm = pd.DataFrame(columns=self.citac.expectedColsSpan)
            ...
            for d in range(timeRaspon + 1):
                ...
                if len(koncFrejm):
                    master_konc_frm = master_konc_frm.append(koncFrejm)
                if len(zeroFrejm):
                    master_zero_frm = master_zero_frm.append(zeroFrejm)
                if len(spanFrejm):
                    master_span_frm = master_span_frm.append(spanFrejm)
...
class RESTZahtjev(object):
    ...
    def get_statusMap(self):
        ...
        """
        head = {"accept": "application/json"}
        jsonStr = self._get_request(self.status_map_url, '', head)
